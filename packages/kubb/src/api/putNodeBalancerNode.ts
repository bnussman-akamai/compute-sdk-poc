 ...
 */
export async function putNodeBalancerNode(apiVersion: PutNodeBalancerNodePathParams["apiVersion"], nodeBalancerId: PutNodeBalancerNodePathParams["nodeBalancerId"], configId: PutNodeBalancerNodePathParams["configId"], nodeId: PutNodeBalancerNodePathParams["nodeId"], data?: PutNodeBalancerNodeMutationRequest, config: Partial<RequestConfig<PutNodeBalancerNodeMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutNodeBalancerNodeMutationResponse, ResponseErrorConfig<Error>, PutNodeBalancerNodeMutationRequest>({ method : "PUT", url : getPutNodeBalancerNodeUrl(apiVersion, nodeBalancerId, configId, nodeId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}