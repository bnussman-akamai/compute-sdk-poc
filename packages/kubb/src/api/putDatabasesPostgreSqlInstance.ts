...
function getPutDatabasesPostgreSqlInstanceUrl(apiVersion: PutDatabasesPostgreSqlInstancePathParams["apiVersion"], instanceId: PutDatabasesPostgreSqlInstancePathParams["instanceId"]) {
  const res = { method: 'PUT', url: `/${apiVersion}/databases/postgresql/instances/${instanceId}` as const }  
  return res
}
 ...
 */
export async function putDatabasesPostgreSqlInstance(apiVersion: PutDatabasesPostgreSqlInstancePathParams["apiVersion"], instanceId: PutDatabasesPostgreSqlInstancePathParams["instanceId"], data?: PutDatabasesPostgreSqlInstanceMutationRequest, config: Partial<RequestConfig<PutDatabasesPostgreSqlInstanceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutDatabasesPostgreSqlInstanceMutationResponse, ResponseErrorConfig<Error>, PutDatabasesPostgreSqlInstanceMutationRequest>({ method : "PUT", url : getPutDatabasesPostgreSqlInstanceUrl(apiVersion, instanceId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}