 ...
 * {@link /:apiVersion/linode/instances/:linodeId/migrate}
 */
export async function postMigrateLinodeInstance(apiVersion: PostMigrateLinodeInstancePathParams["apiVersion"], linodeId: PostMigrateLinodeInstancePathParams["linodeId"], data?: PostMigrateLinodeInstanceMutationRequest, config: Partial<RequestConfig<PostMigrateLinodeInstanceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostMigrateLinodeInstanceMutationResponse, ResponseErrorConfig<Error>, PostMigrateLinodeInstanceMutationRequest>({ method : "POST", url : getPostMigrateLinodeInstanceUrl(apiVersion, linodeId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}