 ...
 */
export async function postDomainRecord(apiVersion: PostDomainRecordPathParams["apiVersion"], domainId: PostDomainRecordPathParams["domainId"], data: PostDomainRecordMutationRequest, config: Partial<RequestConfig<PostDomainRecordMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostDomainRecordMutationResponse, ResponseErrorConfig<Error>, PostDomainRecordMutationRequest>({ method : "POST", url : getPostDomainRecordUrl(apiVersion, domainId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}