 ...
 */
export async function putFirewallRules(apiVersion: PutFirewallRulesPathParams["apiVersion"], firewallId: PutFirewallRulesPathParams["firewallId"], data?: PutFirewallRulesMutationRequest, config: Partial<RequestConfig<PutFirewallRulesMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutFirewallRulesMutationResponse, ResponseErrorConfig<Error>, PutFirewallRulesMutationRequest>({ method : "PUT", url : getPutFirewallRulesUrl(apiVersion, firewallId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}