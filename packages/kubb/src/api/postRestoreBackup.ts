...
function getPostRestoreBackupUrl(apiVersion: PostRestoreBackupPathParams["apiVersion"], linodeId: PostRestoreBackupPathParams["linodeId"], backupId: PostRestoreBackupPathParams["backupId"]) {
  const res = { method: 'POST', url: `/${apiVersion}/linode/instances/${linodeId}/backups/${backupId}/restore` as const }  
  return res
}
 ...
 * {@link /:apiVersion/linode/instances/:linodeId/backups/:backupId/restore}
 */
export async function postRestoreBackup(apiVersion: PostRestoreBackupPathParams["apiVersion"], linodeId: PostRestoreBackupPathParams["linodeId"], backupId: PostRestoreBackupPathParams["backupId"], data: PostRestoreBackupMutationRequest, config: Partial<RequestConfig<PostRestoreBackupMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostRestoreBackupMutationResponse, ResponseErrorConfig<Error>, PostRestoreBackupMutationRequest>({ method : "POST", url : getPostRestoreBackupUrl(apiVersion, linodeId, backupId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}