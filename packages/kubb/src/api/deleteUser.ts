 ...
 */
export async function deleteUser(apiVersion: DeleteUserPathParams["apiVersion"], username: DeleteUserPathParams["username"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const res = await request<DeleteUserMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "DELETE", url : getDeleteUserUrl(apiVersion, username).url.toString(), ... requestConfig })  
  return res.data
}