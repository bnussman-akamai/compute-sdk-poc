 ...
 */
export async function putAccount(apiVersion: PutAccountPathParams["apiVersion"], data?: PutAccountMutationRequest, config: Partial<RequestConfig<PutAccountMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutAccountMutationResponse, ResponseErrorConfig<Error>, PutAccountMutationRequest>({ method : "PUT", url : getPutAccountUrl(apiVersion).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}