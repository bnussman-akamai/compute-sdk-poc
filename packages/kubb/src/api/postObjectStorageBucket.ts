 ...
 */
export async function postObjectStorageBucket(apiVersion: PostObjectStorageBucketPathParams["apiVersion"], data: PostObjectStorageBucketMutationRequest, config: Partial<RequestConfig<PostObjectStorageBucketMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostObjectStorageBucketMutationResponse, ResponseErrorConfig<Error>, PostObjectStorageBucketMutationRequest>({ method : "POST", url : getPostObjectStorageBucketUrl(apiVersion).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}