 ...
 */
export async function postProfilePhoneNumber(apiVersion: PostProfilePhoneNumberPathParams["apiVersion"], data: PostProfilePhoneNumberMutationRequest, config: Partial<RequestConfig<PostProfilePhoneNumberMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostProfilePhoneNumberMutationResponse, ResponseErrorConfig<Error>, PostProfilePhoneNumberMutationRequest>({ method : "POST", url : getPostProfilePhoneNumberUrl(apiVersion).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}