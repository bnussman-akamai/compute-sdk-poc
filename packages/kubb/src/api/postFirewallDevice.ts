...
function getPostFirewallDeviceUrl(apiVersion: PostFirewallDevicePathParams["apiVersion"], firewallId: PostFirewallDevicePathParams["firewallId"]) {
  const res = { method: 'POST', url: `/${apiVersion}/networking/firewalls/${firewallId}/devices` as const }  
  return res
}
 ...
 */
export async function postFirewallDevice(apiVersion: PostFirewallDevicePathParams["apiVersion"], firewallId: PostFirewallDevicePathParams["firewallId"], data: PostFirewallDeviceMutationRequest, config: Partial<RequestConfig<PostFirewallDeviceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostFirewallDeviceMutationResponse, ResponseErrorConfig<Error>, PostFirewallDeviceMutationRequest>({ method : "POST", url : getPostFirewallDeviceUrl(apiVersion, firewallId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}